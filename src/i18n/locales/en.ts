import type { TranslationKey } from './de';

export const en: Record<TranslationKey, string> = {
  'common.loading': 'Loading…',
  'common.back': 'Back',
  'common.close': 'Close',
  'common.cancel': 'Cancel',
  'common.save': 'Save',
  'language.sr': 'Serbian',
  'language.de': 'German',
  'language.fr': 'French',
  'language.en': 'English',
  'language.switch': 'Choose language',
  'header.brand': 'My Task Manager',
  'header.logout': 'Log out',
  'login.title': 'Welcome back',
  'login.titleOtp': 'Check your email',
  'login.subtitle': 'Sign in and stay focused.',
  'login.brand.tagline': 'MASTER YOURSELF',
  'login.hero.title': 'Discipline beats motivation.',
  'login.hero.subtitle': 'Grow every day.',
  'login.otpHint': 'We will send you a one-time login code.',
  'login.security': 'Your data is safe and protected.',
  'login.levels.compactTitle': '24 levels. 24 months.',
  'login.levels.compactSubtitle': 'A stronger version of you.',
  'login.levels.swipeHint': 'Swipe to explore all levels',
  'login.subtitleOtp': 'We sent a code to {{email}}.',
  'login.email': 'Email address',
  'login.emailPlaceholder': 'name@domain.com',
  'login.otp': 'OTP code',
  'login.otpPlaceholder': '123456',
  'login.sendCode': 'Send code',
  'login.sending': 'Sending…',
  'login.submit': 'Sign in',
  'login.checking': 'Checking…',
  'login.errorOtpRequest': 'Could not send OTP.',
  'login.errorOtpRequestDev':
    'OTP failed ({{url}}). Is the backend reachable? Check the IP in the dev panel or switch to production.',
  'login.errorLogin': 'OTP is invalid or expired.',
  'login.levels.title': 'Mastery levels',
  'login.levels.hintHighlight': '24 levels',
  'login.levels.hintRest': ' – one step further every 30 days.',
  'login.levels.moreLevels': '+{{count}} more levels',
  'login.help.trigger': 'Show sign-in instructions',
  'login.help.title': 'How sign-in works',
  'login.help.step1':
    'Enter your email address and tap “Send code” to receive a one-time sign-in code.',
  'login.help.step2': 'Check your inbox and enter the code on the next screen.',
  'login.help.step3':
    'Use this code to sign in and open the application right away.',
  'login.help.note':
    'Whenever you use the same email address, your saved tasks will be available. No password is required.',
  'home.welcome': 'You are signed in.',
  'home.placeholder': 'More screens coming soon.',
  'nav.today': 'Today',
  'nav.tasks': 'Tasks',
  'nav.progress': 'Progress',
  'nav.profile': 'Profile',
  'nav.backToToday': 'Back to Today',
  'dev.api.title': 'Backend (Dev)',
  'dev.api.hintLocal':
    'Local — same data as web at localhost:5173 (test accounts with premium).',
  'dev.api.hintProduction':
    'Production — live Railway data (prod accounts).',
  'dev.api.switchToProduction': 'Switch to production',
  'dev.api.switchToLocal': 'Switch to local',
  'dev.api.devHostLabel': 'Mac IP (LAN)',
  'dev.api.devHostPlaceholder': '192.168.178.29',
  'dev.api.saveDevHost': 'Save IP',
  'dev.api.invalidHost': 'Invalid IP address.',
  'recurring.loading': 'Loading tasks…',
  'recurring.error': 'Could not load tasks.',
  'recurring.progressError': 'Could not load progress.',
  'recurring.levelsError': 'Could not load levels.',
  'recurring.premiumRequired': 'Premium required for daily tasks.',
  'recurring.noTasks': 'No daily tasks yet.',
  'recurring.addTaskDaily': 'New daily task',
  'recurring.status.toggle': 'Change status',
  'recurring.status.toggleHint': 'Tap the name or checkbox to change status',
  'recurring.edit': 'Edit',
  'recurring.delete': 'Delete',
  'recurring.deleteTitle': 'Delete task',
  'recurring.deleteConfirm':
    'Delete “{{title}}”? This action cannot be undone.',
  'recurring.deleteBlockedDone':
    'Completed tasks (status “{{column}}”) cannot be deleted. Set the task to Open or In progress first.',
  'recurring.deleteError': 'Could not delete task.',
  'recurring.deleteErrorDoneColumn':
    'Completed tasks (status “{{column}}”) cannot be deleted. Set the task to Open or In progress first.',
  'recurring.streak.count': 'Streak {{count}}',
  'recurring.status.todo': 'Open',
  'recurring.status.inProgress': 'In progress',
  'recurring.status.done': 'Done',
  'recurring.stats.dailyHint': 'Daily task',
  'recurring.loadingTask': 'Loading task…',
  'recurring.stats.currentStreak': 'Current streak',
  'recurring.stats.highestStreak': 'Longest streak',
  'recurring.stats.currentLevel': 'Current level',
  'recurring.stats.highestLevel': 'Highest level',
  'recurring.stats.daysValue': '{{days}} days',
  'recurring.stats.levelValue': 'Level {{level}}',
  'recurring.profile.title': 'Mastery profile',
  'recurring.profile.levelLabel': 'LEVEL {{level}}',
  'recurring.profile.unknownLevel': 'Unknown level',
  'recurring.profile.daysToNext': '{{current}} / {{total}} days to level {{next}}',
  'recurring.profile.maxLevel': 'Maximum level reached',
  'recurring.today.label': 'TODAY',
  'recurring.today.remaining': '{{count}} tasks left',
  'recurring.today.remainingOne': '1 task left',
  'recurring.today.allDone': 'All done!',
  'recurring.today.resetAt': 'Reset at 00:05',
  'recurring.today.doneLabel': 'done',
  'recurring.tasks.sectionTitle': 'Daily tasks',
  'recurring.focus.title': 'Keep your focus.',
  'recurring.banner.successLine': 'Great! Your streak stays intact until reset.',
  'recurring.banner.warningLine':
    'Complete all tasks before the daily reset to keep your streak & level.',
  'recurring.celebration.title': 'Knockout! All done!',
  'recurring.celebration.subtitle':
    'Come back tomorrow, complete your tasks, and push your streak to new heights.',
  'recurring.celebration.dismiss': 'Tap to close',
  'recurring.form.createTitle': 'New task',
  'recurring.form.editTitle': 'Edit task',
  'recurring.form.titleLabel': 'Title',
  'recurring.form.titlePlaceholder': 'What should be done regularly?',
  'recurring.form.descriptionLabel': 'Description',
  'recurring.form.descriptionPlaceholder': 'Optional details…',
  'recurring.form.statusLabel': 'Status',
  'recurring.form.titleRequiredHint': 'A task cannot be saved without a title.',
  'recurring.form.titleRequired':
    'Please enter a title — a task without a name cannot be saved.',
  'recurring.form.save': 'Save',
  'recurring.form.error': 'Could not save.',
  'tasks.sectionTitle': 'TASKS',
  'tasks.loading': 'Loading tasks…',
  'tasks.loadingTask': 'Loading task…',
  'tasks.error': 'Could not load tasks.',
  'tasks.noTasks': 'No tasks yet.',
  'tasks.noTasksFiltered': 'No tasks in this filter.',
  'tasks.addTask': 'New task',
  'tasks.edit': 'Edit',
  'tasks.delete': 'Delete',
  'tasks.deleteTitle': 'Delete task',
  'tasks.deleteConfirm':
    'Delete “{{title}}”? This action cannot be undone.',
  'tasks.deleteError': 'Could not delete task.',
  'tasks.status.todo': 'Open',
  'tasks.status.inProgress': 'In progress',
  'tasks.status.review': 'Review',
  'tasks.status.done': 'Done',
  'tasks.filter.all': 'All',
  'tasks.filter.importantUrgent': 'Important & urgent',
  'tasks.filter.important': 'Important',
  'tasks.filter.urgent': 'Urgent',
  'tasks.filter.other': 'Other',
  'tasks.deadline.overdue': 'OVERDUE',
  'tasks.deadline.thisWeek': 'THIS WEEK',
  'tasks.priority.none': 'Other',
  'tasks.priority.important': 'Important',
  'tasks.priority.urgent': 'Urgent',
  'tasks.priority.importantUrgent': 'Important & urgent',
  'tasks.form.createTitle': 'New task',
  'tasks.form.editTitle': 'Edit task',
  'tasks.form.titleLabel': 'Title',
  'tasks.form.titlePlaceholder': 'What needs to be done?',
  'tasks.form.descriptionLabel': 'Description',
  'tasks.form.descriptionPlaceholder': 'Optional details…',
  'tasks.form.statusLabel': 'Status',
  'tasks.form.priorityLabel': 'Priority',
  'tasks.form.deadlineLabel': 'Deadline',
  'tasks.form.deadlinePlaceholder': '2026-08-19T13:41',
  'tasks.form.titleRequired': 'Please enter a title.',
  'tasks.form.deadlineInvalid': 'Invalid date format.',
  'tasks.form.save': 'Save',
  'tasks.form.error': 'Could not save.',
  'progress.sectionTitle': 'PROGRESS',
  'progress.last7Days': 'LAST 7 DAYS',
  'progress.remainingToday': '{{count}} tasks left today',
  'progress.remainingTodayOne': '1 task left today',
  'progress.allDoneToday': 'All done today!',
  'progress.nextGoal': 'NEXT GOAL',
  'progress.nextGoalTitle': 'Level {{level}} • {{name}}',
  'progress.daysToNextLevel': '{{count}} successful days left',
  'progress.nextMilestone': 'NEXT MILESTONE',
  'progress.recordInSight': 'NEW RECORD IN SIGHT',
  'progress.daysToRecord': '{{count}} days until your longest streak.',
  'progress.recordAchieved': 'New record! Congratulations!',
  'premium.badge': 'Premium',
  'premium.title': 'Unlock your full potential',
  'premium.subtitle':
    'Daily tasks, streaks and 24 levels help you become better every day.',
  'premium.feature.recurring.title': 'Become the best version of yourself.',
  'premium.feature.recurring.description': 'Master your habits.',
  'premium.feature.streaks.title': 'Build streaks',
  'premium.feature.streaks.description':
    'Stay consistent, grow your chain and never lose your progress.',
  'premium.feature.levels.title': 'Reach Level 24.',
  'premium.feature.levels.description': 'Turn small habits into a new life.',
  'premium.feature.secure.title': 'Your progress is safe',
  'premium.feature.secure.description':
    'Your data, streaks and achievements are stored permanently.',
  'premium.feature.access.title': 'Access to web and mobile app',
  'premium.feature.access.description':
    'Use Today everywhere — on desktop and on your phone.',
  'premium.guaranteeMonthly':
    'Cancel monthly anytime. No hidden fees.',
  'premium.guaranteeLifetime':
    'Lifetime: pay once, use forever.',
  'premium.footer': 'Regular tasks stay free for you.',
  'premium.cta': 'Unlock Premium',
  'recurring.premiumUnlockCta': 'Unlock Premium',
  'subscription.checkStatus.cta': 'Check status again',
  'subscription.settings.title': 'Your Premium subscription',
  'subscription.settings.subtitle': 'Manage your plan and view your Premium status.',
  'subscription.settings.loading': 'Loading subscription status…',
  'subscription.settings.error': 'Could not load subscription status.',
  'subscription.settings.statusLabel': 'Status',
  'subscription.settings.status.inactive': 'No active subscription',
  'subscription.settings.status.pending': 'Pending',
  'subscription.settings.status.active': 'Active',
  'subscription.settings.status.trialing': 'Trial',
  'subscription.settings.status.gracePeriod': 'Grace period',
  'subscription.settings.status.pastDue': 'Payment overdue',
  'subscription.settings.status.canceled': 'Canceled',
  'subscription.settings.status.expired': 'Expired',
  'subscription.settings.status.revoked': 'Revoked',
  'subscription.settings.planLabel': 'Plan',
  'subscription.settings.plan.monthly': 'Premium – Monthly',
  'subscription.settings.plan.yearly': 'Premium – Yearly',
  'subscription.settings.plan.lifetime': 'Premium – Lifetime',
  'subscription.settings.billingDateLabel': 'Next billing date',
  'subscription.settings.billingDateUnavailable': 'Not available yet',
  'subscription.settings.cancellationLabel': 'Cancellation',
  'subscription.settings.cancellationExpiresWithDate':
    'Your subscription was canceled and expires on {{date}}.',
  'subscription.settings.cancellationExpires':
    'Your subscription was canceled and expires at the end of the current period.',
  'subscription.settings.syncFailedNotice':
    'Stripe is currently unavailable. The status shown is based on the last known data.',
  'subscription.settings.subscribeCta': 'Unlock Premium',
  'subscription.settings.subscribeHint':
    'No Premium yet? Start your subscription here.',
  'subscription.settings.manageCta': 'Manage subscription',
  'subscription.settings.manageLoading': 'Opening subscription management…',
  'subscription.settings.manageErrorNoCustomer':
    'No manageable web subscription (Stripe) is linked to this account yet.',
  'subscription.settings.manageErrorOpenFailed':
    'Could not open subscription management. Please try again.',
  'subscription.settings.manageErrorTestStore':
    'Test Store purchases only exist in the RevenueCat sandbox — there is no real Google Play subscription page. Use a Play Store or release build to manage a real subscription.',
  'subscription.mobile.provider.stripe': 'Premium is active through your web subscription (Stripe).',
  'subscription.mobile.provider.apple': 'Premium is active through the Apple App Store.',
  'subscription.mobile.provider.googlePlay': 'Premium is active through Google Play.',
  'subscription.mobile.stripeActive':
    'You already have Premium through your web subscription. No additional in-app purchase is needed.',
  'subscription.mobile.alreadyPremium':
    'Premium is already active for this account. No additional purchase is needed.',
  'subscription.mobile.purchaseMonthly': 'Monthly – {{price}}',
  'subscription.mobile.purchaseLifetime': 'Lifetime – {{price}}',
  'subscription.mobile.restore': 'Restore purchases',
  'subscription.mobile.offeringsLoading': 'Loading store offerings…',
  'subscription.mobile.offeringsError':
    'Store offerings could not be loaded. Please check the RevenueCat configuration.',
  'subscription.mobile.offeringsErrorPlayProducts':
    'RevenueCat has no Google Play products in the current offering. Import Play products in the dashboard and assign them to the offering (not Test Store only).',
  'subscription.mobile.offeringsErrorEmptyOffering':
    'The RevenueCat offering is empty or has no packages for monthly/lifetime.',
  'subscription.mobile.revenueCatUnavailable':
    'In-app purchases are not configured on this device yet. The app remains usable.',
  'subscription.mobile.panelUnavailable':
    'The subscription section could not be loaded right now. RevenueCat errors do not block the app — please reload or try again later.',
  'subscription.mobile.purchaseError':
    'The purchase could not be completed. Please try again.',
  'subscription.mobile.restoreError':
    'Purchases could not be restored. Please try again.',
  'subscription.mobile.restoreNothing': 'No restorable purchases were found.',
  'subscription.mobile.purchaseInFlight': 'A purchase is already in progress.',
  'subscription.mobile.identityMismatch':
    'RevenueCat is not linked to your account yet. Wait a moment, sign in again, then retry.',
  'subscription.mobile.processing.title': 'Purchase successful – activation is processing',
  'subscription.mobile.processing.message':
    'Your purchase was successful. Premium will activate once the server confirms it.',
  'subscription.mobile.processing.hint':
    'The store purchase reached RevenueCat. Your backend still needs to confirm it via webhook or sync. Tap “Check status again” or wait a moment.',
  'subscription.mobile.syncFailedRevenueCat':
    'RevenueCat is currently unavailable. The status shown is based on the last known data.',
  'subscription.mobile.syncFailedBoth':
    'Stripe and RevenueCat are currently unavailable. The status shown is based on the last known data.',
  'profile.signedInAs': 'Signed in as',
  'profile.emailUnavailable': 'Email unavailable',
  'profile.session.title': 'Session',
  'profile.logout.hint': 'Sign out of your account on this device',
  'profile.accountMenu.title': 'Account',
  'profile.accountMenu.subscription': 'Subscription & billing',
  'profile.accountMenu.subscriptionHint': 'Premium status, purchase and restore',
};
